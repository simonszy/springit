...
//        model.addAttribute("success", false);
    ...
    public String registerNewUser(@Valid Person user, BindingResult bindingResult, Model model, RedirectAttributes redirectAttributes) {
        if( bindingResult.hasErrors() ) {
            ...
            Person newUser = userService.register(user);
            redirectAttributes
    ...
    public String activate(@PathVariable String email, @PathVariable String activationCode) {
        Optional<Person> user = userService.findByEmailAndActivationCode(email, activationCode);
        if (user.isPresent()) {
